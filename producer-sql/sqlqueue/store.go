...
// WithTracerProvider records a span per database statement. Postgres only.
...
func Open(ctx context.Context, dsn string, opts ...OpenOption) (*Store, error) {
	...
	case DialectPostgres:
		cfg, perr := pgx.ParseConfig(dsn)
		if perr != nil {
			return nil, fmt.Errorf("sqlqueue: parse postgres dsn: %w", perr)
		}
	...
	case DialectSQLite:
		if o.tracerProvider != nil {
			return nil, errors.New("sqlqueue: tracing is only supported on postgres")
		}
		db, err = sql.Open("sqlite", sqliteDSN(dsn))
		if err != nil {
			return nil, fmt.Errorf("sqlqueue: open sqlite: %w", err)
		}
...
func NewStore(ctx context.Context, db *sql.DB, dialect Dialect) (*Store, error) {
	s := &Store{db: db, dialect: dialect, tracer: noop.NewTracerProvider().Tracer("")}
