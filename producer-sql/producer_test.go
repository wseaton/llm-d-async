...
func newProducer(t *testing.T) (*Producer, *sqlqueue.Store) {
	...
	store, err := sqlqueue.Open(ctx, "sqlite://"+filepath.Join(t.TempDir(), "queue.db"))
...
func message(id string) *api.RequestMessage {
	return &api.RequestMessage{ID: id, Created: time.Now().Unix(), Deadline: time.Now().Add(time.Hour).Unix(), Payload: map[string]any{"prompt": id}}
}
